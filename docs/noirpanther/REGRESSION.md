...
| 38  | Downloaded → tap a row, then swipe one            | A tap lights the row for a readable moment before the book opens; a swipe must NOT light it at all. The highlight comes from a CONFIRMED press, not touch-down — touch-down is where a swipe starts too. Measure it: `simctl io recordVideo`, `ffmpeg -vf fps=30`, count frames matching the highlight colour exactly (matching on brightness counts the white text sliding past) |
| 39  | Downloaded → read the row's second line           | "EPUB · 1.0 MB" must be grey, not black-on-black. `color: undefined` in a style does NOT leave a NativeWind variant's colour alone — it overrides it with the default. Boost the screenshot's dark end to check; at (0,0,0) on an (11,10,16) row the eye sees nothing at all                                                                                                      |
| 40  | Switch to a LIGHT theme, then press a row         | The highlight must be visible there too. `surfaceHover` is 1.09 from the background in the light themes — nothing; the row uses `pressedSurface`, which mixes the theme's text into its background. Any new colour goes through every theme as a number, not through one theme by eye                                                                                             |
| 41  | Reader → tap the middle to show the bar           | The page under the bar is BLURRED, not merely darkened — the controls must not have text running through them. Check it, don't assume: a mask or an opacity on the blur silently turns it back into a plain wash. On Android there is no blur by design                                                                                                                           |
