...
| 38  | Downloaded → tap a row, then swipe one            | A tap lights the row for a readable moment before the book opens (React Native's own highlight lasted one frame in a colour you cannot see against this background). A swipe must NOT light it: a press begins the instant a finger lands, so the gesture has to take the highlight back when it claims the touch. Both are measurable off `simctl io recordVideo` — count the lit frames |
