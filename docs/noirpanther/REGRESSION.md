...
| 33  | Downloaded → swipe a row right-to-left            | The Delete button appears and the book does NOT open. Until 2026-09-21 the same gesture did both: a Pressable measures a press by where the finger LIFTS, and a swipe across a full-width row lifts inside it. Long press must still show the same confirmation, a plain tap must still open the book, and a tap on an already-open row must close it |
