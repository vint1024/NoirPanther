...
- **Clean titles and comic languages from file metadata**: EPUBs with several `dc:title` elements (main / subtitle / full title) get the main title instead of a multi-line mash-up; `LanguageISO` from `ComicInfo.xml` is read (upstream only accepts a non-standard `Language` tag)
