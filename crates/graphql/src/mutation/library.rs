...
impl LibraryMutation {
	...
	async fn patch_library(
		...
		// NoirPanther: libraries may span several roots (A1), so validate the whole set
		let existing_extra_paths =
			library_path::Entity::fetch_for_library(core.conn.as_ref(), id.as_str())
				.await?;
		enforce_valid_library_roots(
			core.conn.as_ref(),
			&enforcement_path,
