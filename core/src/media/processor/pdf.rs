use std::{
	collections::HashMap,
	io::Cursor,
	path::{Path, PathBuf},
	sync::{Mutex, MutexGuard, OnceLock},
};

use models::shared::image_processor_options::SupportedImageFormat;
use pdfium_render::prelude::{
	PdfColor, PdfDocument, PdfDocumentMetadataTagType, PdfPageIndex, PdfRenderConfig,
	Pdfium,
};

use crate::{
	config::StumpConfig,
	fs_utils::{
		archive::create_zip_archive,
		hash::{self, generate_koreader_hash},
		ContentType, FileParts, PathUtils,
	},
	image::into_image_format,
	media::processor::{
		error::MediaProcessorError, AnalyzedPage, GeneratedFileHashes, MediaProcessor,
		MediaProcessorOptions, ProcessedMediaFile,
	},
	metadata::ProcessedMediaMetadata,
};

/// alright so tldr; this is a temporary workaround (the mutex) until upstream fixes issues with
/// the `thread_safe` feature that make it unsafe (lol).
/// im subscribed to the following and will remove once able after resolved:
/// - https://github.com/ajrcarey/pdfium-render/issues/262
static PDFIUM: OnceLock<Result<Mutex<Pdfium>, MediaProcessorError>> = OnceLock::new();

pub struct PdfProcessor {
	config: StumpConfig,
}

impl PdfProcessor {
	pub fn new(config: StumpConfig) -> Self {
		Self { config }
	}
}

impl MediaProcessor for PdfProcessor {
	fn generate_stump_hash(&self, path: &Path) -> Result<String, MediaProcessorError> {
		let size = std::fs::metadata(path)?.len();
		if size < 10 {
			return Err(MediaProcessorError::FileTooSmall);
		}
		Ok(hash::generate(path, size / 10)?)
	}

	fn generate_hashes(
		&self,
		path: &Path,
		MediaProcessorOptions {
			generate_file_hashes,
			generate_koreader_hashes,
			..
		}: MediaProcessorOptions,
	) -> Result<GeneratedFileHashes, MediaProcessorError> {
		let stump_hash = generate_file_hashes
			.then(|| self.generate_stump_hash(path))
			.transpose()?;
		let koreader_hash = generate_koreader_hashes
			.then(|| generate_koreader_hash(path))
			.transpose()?;

		Ok(GeneratedFileHashes {
			stump: stump_hash,
			koreader: koreader_hash,
		})
	}

	fn process_metadata(
		&self,
		path: &Path,
	) -> Result<Option<ProcessedMediaMetadata>, MediaProcessorError> {
		process_metadata_internal(path, &self.config.pdfium_path)
	}

	fn process(
		&self,
		path: &Path,
		options: MediaProcessorOptions,
	) -> Result<ProcessedMediaFile, MediaProcessorError> {
		// TODO: undo this once upstream fixes the issue outlined above
		// this is scoped a bit more tightly than before since wrapping the singleton
		// in a mutex. to extract metadata would mean attempting to obtain a lock on
		// a locked mutex (which is bad)
		let (pages, metadata) = {
			let pdfium = renderer(&self.config.pdfium_path)?;
			let document =
				pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None)?;
			let pages = document.pages().len();
			let metadata = if options.process_metadata {
				extract_metadata_from_open_document(&document)?
			} else {
				None
			};
			(pages, metadata)
		};

		let hashes = self.generate_hashes(path, options)?;

		Ok(ProcessedMediaFile {
			path: path.to_path_buf(),
			hash: hashes.stump,
			koreader_hash: hashes.koreader,
			metadata,
			pages,
		})
	}

	fn get_page(
		&self,
		path: &Path,
		page: i32,
	) -> Result<(ContentType, Vec<u8>), MediaProcessorError> {
		render_page_with_quality(path, page, &self.config, false)
	}

	fn get_page_count(&self, path: &Path) -> Result<i32, MediaProcessorError> {
		pdf_page_count(path, &self.config.pdfium_path)
	}

	fn get_page_content_types(
		&self,
		_: &Path,
		pages: Vec<i32>,
	) -> Result<HashMap<i32, ContentType>, MediaProcessorError> {
		let content_type = ContentType::from(self.config.get_pdf_render_format());
		Ok(pages.into_iter().map(|page| (page, content_type)).collect())
	}

	fn analyze_page(
		&self,
		path: &Path,
		page: i32,
	) -> Result<AnalyzedPage, MediaProcessorError> {
		let pdfium = renderer(&self.config.pdfium_path)?;
		let document =
			pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None)?;

		let total_pages = document.pages().len() as usize;
		if page < 1 || page as usize > total_pages {
			return Err(MediaProcessorError::PageNotFound);
		}

		let size = document.pages().page_size(((page - 1) as u16).into())?;
		let width = size.width().value as u32;
		let height = size.height().value as u32;

		let output_format = self.config.get_pdf_render_format();
		let content_type = ContentType::from(output_format);

		Ok(AnalyzedPage {
			width,
			height,
			content_type,
		})
	}
}

/// Extract metadata from a PDF file provided an already-open PdfDocument
fn extract_metadata_from_open_document(
	document: &PdfDocument<'_>,
) -> Result<Option<ProcessedMediaMetadata>, MediaProcessorError> {
	let mut metadata_map: HashMap<String, Vec<String>> = HashMap::new();
	for tag in document.metadata().iter() {
		let value_ref = tag.value();
		if value_ref.is_empty() {
			continue;
		}
		let value = value_ref.to_string();
		let key = match tag.tag_type() {
			PdfDocumentMetadataTagType::Title => "title".to_string(),
			PdfDocumentMetadataTagType::Author => "author".to_string(),
			PdfDocumentMetadataTagType::Subject => "summary".to_string(),
			PdfDocumentMetadataTagType::Keywords => "tags".to_string(),
			PdfDocumentMetadataTagType::CreationDate => "date".to_string(),
			PdfDocumentMetadataTagType::ModificationDate => "date".to_string(),
			_ => continue,
		};
		metadata_map.entry(key).or_default().push(value);
	}

	Ok(if metadata_map.is_empty() {
		None
	} else {
		Some(ProcessedMediaMetadata::from(metadata_map))
	})
}

/// Process the metadata of a PDF file using the specified config/env pdfium path.
pub fn process_metadata_internal(
	path: &Path,
	pdfium_path: &Option<String>,
) -> Result<Option<ProcessedMediaMetadata>, MediaProcessorError> {
	let pdfium = renderer(pdfium_path)?;
	let document = pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None)?;
	extract_metadata_from_open_document(&document)
}

/// Returns a mutex-guarded pdfium singleton, initializing it on first call
///
/// Note that a failed init is cached permanently and requires a restart the process to retry
/// initialization
///
/// See: https://github.com/ajrcarey/pdfium-render#thread-safety
pub fn renderer(
	pdfium_path: &Option<String>,
) -> Result<MutexGuard<'static, Pdfium>, MediaProcessorError> {
	// TODO: revert to returning a good ol &'static Pdfium once upstream fixes
	// the issue outlined above
	let result = PDFIUM.get_or_init(|| {
		let path = pdfium_path
			.clone()
			.or_else(|| std::env::var("PDFIUM_PATH").ok());

		if let Some(path) = path {
			tracing::info!(path, "Initializing PDFium from provided path");
			let bindings = Pdfium::bind_to_library(&path)
				.or_else(|e| {
					tracing::error!(provided_path = ?path, ?e, "Failed to bind to PDFium library at provided path, attempting fallback to system library");
					Pdfium::bind_to_system_library()
				})
				.map_err(|e| {
					tracing::error!(?e, "Failed to bind to system PDFium library");
					MediaProcessorError::PdfConfiguration
				})?;
			Ok(Mutex::new(Pdfium::new(bindings)))
		} else {
			tracing::warn!(
				"No PDFium path provided, will attempt to bind to system library"
			);
			Pdfium::bind_to_system_library()
				.map(|b| Mutex::new(Pdfium::new(b)))
				.map_err(|e| {
					tracing::error!(?e, "Failed to bind to system PDFium library");
					MediaProcessorError::PdfConfiguration
				})
		}
	});

	result
		.as_ref()
		.map_err(|_| MediaProcessorError::PdfConfiguration)?
		.lock()
		.map_err(|_| {
			tracing::error!(
				"PDFium mutex is poisoned, you will need to restart the process to recover from this error"
			);
			MediaProcessorError::PdfConfiguration
		})
}

/// Core rendering function with configurable quality
pub fn render_page_with_quality(
	path: &Path,
	page: i32,
	config: &StumpConfig,
	force_high_quality: bool,
) -> Result<(ContentType, Vec<u8>), MediaProcessorError> {
	let pdfium = renderer(&config.pdfium_path)?;
	let document = pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None)?;
	let total_pages = document.pages().len() as usize;

	if page < 1 || page as usize > total_pages {
		return Err(MediaProcessorError::PageNotFound);
	}

	let page_index = (page - 1) as PdfPageIndex;
	let document_page = document.pages().get(page_index)?;

	let page_width = document_page.width().value;
	let page_height = document_page.height().value;

	// scale from PDF's 72 DPI base to configured DPI, then clamp to max dimension
	let dpi_scale = config.pdf_render_dpi as f32 / 72.0;
	let mut target_width = page_width * dpi_scale;
	let mut target_height = page_height * dpi_scale;
	let max_dim = config.pdf_max_dimension as f32;
	if max_dim > 0.0 && (target_width > max_dim || target_height > max_dim) {
		let fit_scale = (max_dim / target_width).min(max_dim / target_height);
		target_width *= fit_scale;
		target_height *= fit_scale;
	}

	let target_width = target_width.max(1.0) as i32;
	let target_height = target_height.max(1.0) as i32;

	let use_high_quality = force_high_quality || config.pdf_high_quality;
	let render_config = if use_high_quality {
		PdfRenderConfig::new()
			.set_target_width(target_width)
			.set_maximum_height(target_height)
			.use_print_quality(true)
			.set_image_smoothing(true)
			.set_text_smoothing(true)
			.set_path_smoothing(true)
			.set_clear_color(PdfColor::new(255, 255, 255, 255))
			.clear_before_rendering(true)
	} else {
		// Fast rendering while maintaining text readability
		let fast_width = (target_width * 4 / 5).max(1);
		let fast_height = (target_height * 4 / 5).max(1);
		PdfRenderConfig::new()
			.set_target_width(fast_width)
			.set_maximum_height(fast_height)
			.use_print_quality(false)
			.set_image_smoothing(false)
			.set_text_smoothing(true)
			.set_path_smoothing(false)
			.set_clear_color(PdfColor::new(255, 255, 255, 255))
			.clear_before_rendering(true)
	};

	let bitmap = document_page.render_with_config(&render_config)?;
	let dyn_image = bitmap.as_image()?;

	let output_format = config.get_pdf_render_format();
	let image_format = into_image_format(output_format);
	let content_type = ContentType::from(output_format);

	let image = dyn_image
		.as_rgba8()
		.ok_or(MediaProcessorError::PdfRenderFailed)?;

	let mut buffer = Cursor::new(vec![]);
	image.write_to(&mut buffer, image_format)?;
	Ok((content_type, buffer.into_inner()))
}

/// Async version of get_page with caching support
pub async fn get_page_async(
	path: &Path,
	page: i32,
	config: &StumpConfig,
) -> Result<(ContentType, Vec<u8>), MediaProcessorError> {
	if config.pdf_cache_pages {
		if let Ok(Some(cached)) = get_cached_page(path, page, config).await {
			if config.pdf_prerender_range > 0 {
				let path_owned = path.to_path_buf();
				let config_owned = config.clone();
				tokio::spawn(async move {
					prerender_adjacent_pages(&path_owned, page, &config_owned).await;
				});
			}
			return Ok(cached);
		}
	}

	let path_owned = path.to_path_buf();
	let config_owned = config.clone();

	let result = tokio::task::spawn_blocking(move || {
		render_page_with_quality(&path_owned, page, &config_owned, false)
	})
	.await??;

	if config.pdf_cache_pages {
		// non-fatal, does not rly matter if caching fails
		let _ = cache_page(path, page, &result.1, config).await;
	}

	if config.pdf_cache_pages && config.pdf_prerender_range > 0 {
		let path_owned = path.to_path_buf();
		let config_owned = config.clone();
		tokio::spawn(async move {
			prerender_adjacent_pages(&path_owned, page, &config_owned).await;
		});
	}

	Ok(result)
}

// TODO: part of the reorg and realized this was never actually used, use it or remove it
#[allow(dead_code)]
pub fn convert_to_zip(
	path: &Path,
	delete_source: bool,
	format: Option<SupportedImageFormat>,
	config: &StumpConfig,
) -> Result<PathBuf, MediaProcessorError> {
	let pdfium = renderer(&config.pdfium_path)?;
	let document = pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None)?;

	let chosen_format = format.unwrap_or_else(|| config.get_pdf_render_format());
	let output_format = into_image_format(chosen_format);
	let output_extension = chosen_format.extension();

	let converted_pages = document
		.pages()
		.iter()
		.enumerate()
		.filter_map(|(idx, page)| {
			let render_page = || -> Result<Vec<u8>, MediaProcessorError> {
				let page_width = page.width().value;
				let page_height = page.height().value;

				// scale from PDF's 72 DPI base to configured DPI, then clamp to max dimension
				let dpi_scale = config.pdf_render_dpi as f32 / 72.0;
				let mut target_width = page_width * dpi_scale;
				let mut target_height = page_height * dpi_scale;

				let max_dim = config.pdf_max_dimension as f32;
				if max_dim > 0.0 && (target_width > max_dim || target_height > max_dim) {
					let fit_scale = (max_dim / target_width).min(max_dim / target_height);
					target_width *= fit_scale;
					target_height *= fit_scale;
				}

				let render_config = PdfRenderConfig::new()
					.set_target_width(target_width.max(1.0) as i32)
					.set_maximum_height(target_height.max(1.0) as i32)
					.use_print_quality(true)
					.set_image_smoothing(true)
					.set_text_smoothing(true)
					.set_path_smoothing(true)
					.set_clear_color(PdfColor::new(255, 255, 255, 255))
					.clear_before_rendering(true);

				let bitmap = page.render_with_config(&render_config)?;
				let dyn_image = bitmap.as_image()?;

				let image = dyn_image
					.as_rgba8()
					.ok_or(MediaProcessorError::PdfRenderFailed)?;
				let mut buffer = Cursor::new(vec![]);
				image.write_to(&mut buffer, output_format)?;
				Ok(buffer.into_inner())
			};

			match render_page() {
				Ok(buf) => Some((idx, buf)),
				Err(e) => {
					tracing::error!(error = ?e, "Failed to render PDF page {}", idx + 1);
					None
				},
			}
		})
		.collect::<Vec<(usize, Vec<u8>)>>();

	let FileParts {
		file_name,
		file_stem,
		extension,
	} = path.file_parts();
	let parent = path.parent().unwrap_or_else(|| Path::new("/"));

	let cache_dir = config.cache_directory();
	let unpacked_path = cache_dir.join(&file_stem);

	std::fs::create_dir_all(&unpacked_path)?;

	for (idx, image_buf) in converted_pages {
		let image_path = unpacked_path.join(format!(
			"{}_{:03}.{}",
			file_stem,
			idx + 1,
			output_extension
		));
		if let Err(err) = std::fs::write(&image_path, image_buf) {
			// non-fatal, don't think we should fail the entire conversion if one page fails to write
			tracing::error!(error = ?err, ?image_path, "Failed to write PDF page image to file");
		}
	}

	let zip_path = create_zip_archive(&unpacked_path, &file_name, &extension, parent)?;

	if delete_source {
		// note that the trash::delete won't work in docker, but or_else should hit in that case
		let _ = trash::delete(path).or_else(|_| std::fs::remove_file(path));
	}

	// TODO: guard against accidentally deleting important paths
	if unpacked_path.exists() {
		let _ = std::fs::remove_dir_all(&unpacked_path);
	}

	Ok(zip_path)
}

fn pdf_page_count(
	path: &Path,
	pdfium_path: &Option<String>,
) -> Result<i32, MediaProcessorError> {
	let pdfium = renderer(pdfium_path)?;
	let document = pdfium.load_pdf_from_file(path.to_str().unwrap_or_default(), None)?;
	Ok(document.pages().len())
}

fn generate_cache_key(
	pdf_path: &Path,
	page: i32,
	config: &StumpConfig,
) -> Result<String, MediaProcessorError> {
	let metadata = std::fs::metadata(pdf_path)?;
	let modified_time = metadata
		.modified()?
		.duration_since(std::time::UNIX_EPOCH)
		.map_err(|e| MediaProcessorError::Unknown(e.to_string()))?
		.as_secs();
	let file_size = metadata.len();

	use std::collections::hash_map::DefaultHasher;
	use std::hash::{Hash, Hasher};

	let mut hasher = DefaultHasher::new();
	pdf_path.hash(&mut hasher);
	file_size.hash(&mut hasher);
	modified_time.hash(&mut hasher);
	config.pdf_max_dimension.hash(&mut hasher);
	config.pdf_render_dpi.hash(&mut hasher);
	config.pdf_render_format.hash(&mut hasher);
	config.pdf_high_quality.hash(&mut hasher);
	page.hash(&mut hasher);

	Ok(format!("pdf_{}_{}", hasher.finish(), page))
}

/// Check if a cached page exists and return its content
async fn get_cached_page(
	pdf_path: &Path,
	page: i32,
	config: &StumpConfig,
) -> Result<Option<(ContentType, Vec<u8>)>, MediaProcessorError> {
	if !config.pdf_cache_pages {
		return Ok(None);
	}

	let cache_key = generate_cache_key(pdf_path, page, config)?;
	let output_format = config.get_pdf_render_format();
	let cache_file = config.pdf_cache_directory().join(format!(
		"{}.{}",
		cache_key,
		output_format.extension()
	));

	if cache_file.exists() {
		match tokio::fs::read(&cache_file).await {
			Ok(bytes) => {
				if !bytes.is_empty() {
					return Ok(Some((ContentType::from(output_format), bytes)));
				}
				let _ = tokio::fs::remove_file(&cache_file).await;
			},
			Err(e) => {
				tracing::warn!(cache_file = ?cache_file, error = ?e, "Failed to read cache file");
			},
		}
	}

	Ok(None)
}

/// Save a rendered page to the cache
async fn cache_page(
	pdf_path: &Path,
	page: i32,
	content: &[u8],
	config: &StumpConfig,
) -> Result<(), MediaProcessorError> {
	if !config.pdf_cache_pages || content.is_empty() {
		return Ok(());
	}

	let cache_key = generate_cache_key(pdf_path, page, config)?;
	let cache_dir = config.pdf_cache_directory();

	if !cache_dir.exists() {
		if let Err(e) = tokio::fs::create_dir_all(&cache_dir).await {
			tracing::warn!(cache_dir = ?cache_dir, error = ?e, "Failed to create cache directory");
			return Ok(());
		}
	}

	let output_format = config.get_pdf_render_format();
	let cache_file =
		cache_dir.join(format!("{}.{}", cache_key, output_format.extension()));
	let temp_file =
		cache_dir.join(format!("{}.{}.tmp", cache_key, output_format.extension()));

	match tokio::fs::write(&temp_file, content).await {
		Ok(_) => match tokio::fs::rename(&temp_file, &cache_file).await {
			Ok(_) => {
				tracing::debug!(
					?cache_file,
					size = content.len(),
					"Cached PDF page successfully"
				);
			},
			Err(error) => {
				tracing::warn!(?cache_file, ?error, "Failed to move temp cache file");
				let _ = tokio::fs::remove_file(&temp_file).await;
			},
		},
		Err(error) => {
			tracing::warn!(?cache_file, ?error, "Failed to write to temp cache file");
		},
	}

	Ok(())
}

async fn prerender_adjacent_pages(
	pdf_path: &Path,
	current_page: i32,
	config: &StumpConfig,
) {
	if !config.pdf_cache_pages || config.pdf_prerender_range == 0 {
		return;
	}

	let total_pages = match pdf_page_count(pdf_path, &config.pdfium_path) {
		Ok(count) => count,
		Err(e) => {
			tracing::debug!(
				pdf_path = %pdf_path.display(),
				error = ?e,
				"Failed to get page count for pre-rendering, skipping"
			);
			return;
		},
	};

	let range = config.pdf_prerender_range as i32;
	let start_page = (current_page - range).max(1);
	let end_page = (current_page + range).min(total_pages);

	tracing::debug!(
		pdf_path = %pdf_path.display(),
		current_page,
		start_page,
		end_page,
		total_pages,
		"Pre-rendering adjacent pages"
	);

	// sort closest-first so the most useful pages render first
	let mut pages_to_render: Vec<i32> = (start_page..=end_page)
		.filter(|&page| page != current_page)
		.collect();
	pages_to_render.sort_by_key(|&page| (page - current_page).abs());

	// TODO: once the upstream issue is resolved revisit whether we can parallelize
	// a bit instead of sequentially processing each page
	for page in pages_to_render {
		if let Ok(Some(_)) = get_cached_page(pdf_path, page, config).await {
			continue; // already cached = no work
		}

		let path_clone = pdf_path.to_path_buf();
		let config_clone = config.clone();
		let render_result = tokio::task::spawn_blocking(move || {
			render_page_with_quality(&path_clone, page, &config_clone, true)
		})
		.await;

		match render_result {
			Ok(Ok((_, content))) => {
				if let Err(e) = cache_page(pdf_path, page, &content, config).await {
					tracing::debug!(
						pdf_path = %pdf_path.display(),
						page,
						error = ?e,
						"Failed to cache pre-rendered page"
					);
				} else {
					tracing::debug!(
						pdf_path = %pdf_path.display(),
						page,
						"Pre-rendered and cached page successfully"
					);
				}
			},
			Ok(Err(e)) => {
				tracing::debug!(
					pdf_path = %pdf_path.display(),
					page,
					error = ?e,
					"Failed to render page during pre-rendering"
				);
			},
			Err(e) => {
				tracing::debug!(
					pdf_path = %pdf_path.display(),
					page,
					error = ?e,
					"Pre-rendering task panicked"
				);
			},
		}
	}
}

#[cfg(test)]
mod tests {
	use super::*;
	use crate::media::fixtures::get_test_pdf_path;

	#[test]
	fn test_process() {
		if renderer(&None).is_err() {
			eprintln!("Skipping test: PDFium is not configured or available.");
			return;
		}
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let processor = PdfProcessor::new(config.clone());

		let processed_file = processor.process(
			path,
			MediaProcessorOptions {
				process_metadata: true,
				..Default::default()
			},
		);
		assert!(processed_file.is_ok());
	}

	#[test]
	fn test_get_page_content_types() {
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let processor = PdfProcessor::new(config);

		let content_types = processor.get_page_content_types(path, vec![1]);
		assert!(content_types.is_ok());
	}

	#[test]
	fn test_process_metadata() {
		if renderer(&None).is_err() {
			eprintln!("Skipping test: PDFium is not configured or available.");
			return;
		}
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);

		let metadata = process_metadata_internal(path, &None);
		assert!(metadata.is_ok());
	}

	#[test]
	fn test_get_sample_size() {
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let processor = PdfProcessor::new(config);

		let result = processor.generate_stump_hash(path);
		assert!(result.is_ok());
		assert!(!result.unwrap().is_empty());
	}

	#[test]
	fn test_generate_stump_hash() {
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let processor = PdfProcessor::new(config);

		let hash = processor.generate_stump_hash(path);
		assert!(hash.is_ok());
	}

	#[test]
	fn test_generate_hashes() {
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let processor = PdfProcessor::new(config);

		let hashes = processor.generate_hashes(
			path,
			MediaProcessorOptions {
				generate_file_hashes: true,
				generate_koreader_hashes: true,
				..Default::default()
			},
		);
		assert!(hashes.is_ok());
		let hashes = hashes.unwrap();
		assert!(hashes.stump.is_some());
		assert!(hashes.koreader.is_some());
	}

	#[test]
	fn test_analyze_page() {
		if renderer(&None).is_err() {
			eprintln!("Skipping test: PDFium is not configured or available.");
			return;
		}
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let processor = PdfProcessor::new(config);

		let page = processor.analyze_page(path, 1);
		assert!(page.is_ok());
		let page = page.unwrap();
		assert!(page.width > 0);
		assert!(page.height > 0);
	}

	#[tokio::test]
	async fn test_get_page_async() {
		if renderer(&None).is_err() {
			eprintln!("Skipping test: PDFium is not configured or available.");
			return;
		}
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();

		// Test without cache
		let mut no_cache_config = config.clone();
		no_cache_config.pdf_cache_pages = false;
		let result = get_page_async(path, 1, &no_cache_config).await;
		assert!(result.is_ok());

		// Test with cache enabled
		let mut cache_config = config.clone();
		cache_config.pdf_cache_pages = true;
		let temp_dir = tempfile::tempdir().unwrap();
		cache_config.config_dir = temp_dir.path().to_string_lossy().to_string();

		let result1 = get_page_async(path, 1, &cache_config).await;
		assert!(result1.is_ok());

		// Second call should hit cache and yield identical output
		let result2 = get_page_async(path, 1, &cache_config).await;
		assert!(result2.is_ok());
		assert_eq!(result1.unwrap().1, result2.unwrap().1);
	}

	#[tokio::test]
	async fn test_prerender_adjacent_pages() {
		if renderer(&None).is_err() {
			eprintln!("Skipping test: PDFium is not configured or available.");
			return;
		}
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let mut config = StumpConfig::debug();
		config.pdf_cache_pages = true;
		config.pdf_prerender_range = 1;
		let temp_dir = tempfile::tempdir().unwrap();
		config.config_dir = temp_dir.path().to_string_lossy().to_string();

		// This will pre-render page 2 in the background when requesting page 1
		let result = get_page_async(path, 1, &config).await;
		assert!(result.is_ok());

		// Now check if page 2 was successfully cached (poll to prevent timing flakiness under load)
		let cache_key = generate_cache_key(path, 2, &config).unwrap();
		let output_format = config.get_pdf_render_format();
		let cache_file = config.pdf_cache_directory().join(format!(
			"{}.{}",
			cache_key,
			output_format.extension()
		));

		let mut success = false;
		for _ in 0..100 {
			if cache_file.exists()
				&& cache_file.metadata().map(|m| m.len() > 0).unwrap_or(false)
			{
				success = true;
				break;
			}
			tokio::time::sleep(std::time::Duration::from_millis(100)).await;
		}
		assert!(success);
	}

	#[test]
	fn test_convert_to_zip() {
		if renderer(&None).is_err() {
			eprintln!("Skipping test: PDFium is not configured or available.");
			return;
		}
		let path_str = get_test_pdf_path();
		let path = Path::new(&path_str);
		let config = StumpConfig::debug();
		let result = convert_to_zip(path, false, None, &config);
		assert!(result.is_ok());
		let zip_path = result.unwrap();
		assert!(zip_path.exists());
		if zip_path.exists() {
			let _ = std::fs::remove_file(zip_path);
		}
	}
}
