...
pub fn write_metadata_to_epub(
	...
	if !path.to_lowercase().ends_with(".epub") {
		return Err(MediaProcessorError::UnsupportedFile(
	...
	let result = (|| -> Result<(), MediaProcessorError> {
...
mod tests {
	use super::*;
	use crate::media::{
		fixtures::get_test_epub_path,
		processor::{epub::EpubProcessor, MediaProcessor},
	};
