...
fn get_cover_internal(
	...
	// NoirPanther (A7): the epub has no usable cover image at all (e.g. a text-only
	// export). Generate a placeholder from its metadata instead of erroring, which
	// used to surface as a 500 on every thumbnail request.
	...
	let buffer = generate_cover_placeholder(&title, author.as_deref())
		.map_err(|e| MediaProcessorError::EpubRead(e.to_string()))?;
