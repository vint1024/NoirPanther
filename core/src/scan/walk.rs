	...
	scan::{
		...
		utils::{
			...
			oneshot::{
				...
				PendingOneshotConversion, PreviousOneshotEntry,
			},
...
fn directory_has_media(path: &Path, ignore_rules: &GlobSet) -> bool {
...
fn directory_has_media_deep(path: &Path, ignore_rules: &GlobSet) -> bool {
			...
			walkdir
				...
				.partition_map(|entry| {
					...
					let is_valid = !should_ignore
						&& (check_deep
							&& directory_has_media_deep(entry_path, &ignore_rules)
							|| (!check_deep
								&& directory_has_media(entry_path, &ignore_rules)));
