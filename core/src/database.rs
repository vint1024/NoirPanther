...
pub async fn connect(config: &StumpConfig) -> Result<DatabaseConnection, CoreError> {
	...
	let connection = if connection_url.starts_with("sqlite://") {
		...
		let pool = SqlitePoolOptions::new()
			...
			// pooled connection — content-rule queries and Unicode-aware search
			// depend on them.
			.after_connect(|conn, _meta| {
				Box::pin(
					async move { models::db::register_unicode_functions(conn).await },
				)
			})
		...
		// The fork's string filters and content rules call `ulower(x)` (a custom
		// SQLite function, see models::db). Postgres' built-in lower() is already
		// Unicode-aware, so expose it under the same name to keep the generated
		// SQL backend-agnostic.
		connection
